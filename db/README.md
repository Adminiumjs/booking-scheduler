...
`docker compose up` gives you a working product: `booking-db` creates the
...
By default these talk to the `booking-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/bookings npm run demo:status
```
